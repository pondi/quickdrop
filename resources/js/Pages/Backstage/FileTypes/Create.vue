<template>
  <BackstageLayout>
    <Head title="Add File Type" />
    
    <div class="max-w-2xl mx-auto">
      <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Add File Type</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">Configure a new file type for uploads</p>
      </div>

      <form @submit.prevent="submit" class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 space-y-6">
        <div class="grid grid-cols-2 gap-6">
          <div>
            <label for="extension" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
              File Extension <span class="text-red-500">*</span>
            </label>
            <input
              id="extension"
            ...
            />
            <p v-if="form.errors.extension" class="mt-1 text-sm text-red-600 dark:text-red-400">
              {{ form.errors.extension }}
            </p>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Without the dot (e.g., pdf not .pdf)</p>
          </div>
        ...
        <div class="flex items-center justify-between pt-4">
          ...
          >
            ...
            <span v-else>Create File Type</span>
          </button>
        </div>
      </form>
    </div>
...
const submit = () => {
  form.post(route('backstage.file-types.store'))
}
</script>